...
export default function Marvelcomics() {
      ...
      if (query === "") {
        ...
        if (
          ...
          const result = await axios(
            `http://gateway.marvel.com/v1/public/comics?orderBy=modified&limit=50&ts=1&apikey=239623db6bb1e571ce3329638b9145dc&hash=${hash}`
          );
        ...
        const result = await axios(
          `http://gateway.marvel.com/v1/public/comics?nameStartsWith=${query}&ts=1&apikey=239623db6bb1e571ce3329638b9145dc&hash=${hash}`
        );
        console.log(result.data.data.results);
        setItems(result.data.data.results);
        setLoading(false);
  ...
  return isLoading ? (
    ...
    <MDBCardTitle>
        <h1 style={{margin:'2rem'}}>Marvel Comics</h1>
    </MDBCardTitle>
